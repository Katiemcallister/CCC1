...
router.post('/add', (req, res) => {
  ...
  const createdJobRole = jobRoleService.create(newJobRole);
  res.redirect('/jobroles/' + createdJobRole.id)
});

// Read all jobRoles
router.get('/', (req, res) => {
  const jobRoles = jobRoleService.getAll();
  res.render('jobRole/list', { jobRoles: jobRoles })
});

// Read a jobRole by ID
router.get('/:id', (req, res) => {
  const jobRole = jobRoleService.getById(parseInt(req.params.id));
...
router.post('/update/:id', (req, res) => {
  const updatedJobRole = jobRoleService.update(parseInt(req.params.id), req.body);
...
router.post('/delete/:id', (req, res) => {
  const deletedJobRole = jobRoleService.delete(parseInt(req.params.id));
